...
def _maintenance_arguments() -> list[str]:
    ...
    arguments = [
        ...
        table_path,
        "--app-name",
        f"silver_navigation_events_maintenance",
    ]
...
with DAG(
    dag_id=f"silver_maintenance_navigation_events",
    description=f"Scheduled OPTIMIZE and VACUUM for Silver Delta table: navigation_events",
    start_date=datetime(2026, 1, 1),
    schedule=os.getenv("SILVER_MAINTENANCE_SCHEDULE", "0 1 * * *"),
    catchup=False,
    tags=["silver", "maintenance", "delta", "navigation_events"],
