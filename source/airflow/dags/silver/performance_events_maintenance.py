...
def _maintenance_arguments() -> list[str]:
    ...
    table_path = f"{base_path}/performance_events"
    
    arguments = [
        "--table-path",
        table_path,
        "--app-name",
        f"silver_performance_events_maintenance",
    ]
...
with DAG(
    dag_id=f"silver_maintenance_performance_events",
    description=f"Scheduled OPTIMIZE and VACUUM for Silver Delta table: performance_events",
    start_date=datetime(2026, 1, 1),
    schedule=os.getenv("SILVER_MAINTENANCE_SCHEDULE", "0 1 * * *"),
    catchup=False,
    tags=["silver", "maintenance", "delta", "performance_events"],
