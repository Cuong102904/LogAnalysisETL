...
def _maintenance_arguments() -> list[str]:
    ...
    table_path = f"{base_path}/system_events"
    
    arguments = [
        "--table-path",
        table_path,
        "--app-name",
        f"silver_system_events_maintenance",
    ]
...
with DAG(
    dag_id=f"silver_maintenance_system_events",
    description=f"Scheduled OPTIMIZE and VACUUM for Silver Delta table: system_events",
    start_date=datetime(2026, 1, 1),
    schedule=os.getenv("SILVER_MAINTENANCE_SCHEDULE", "0 1 * * *"),
    catchup=False,
    tags=["silver", "maintenance", "delta", "system_events"],
