...
from tasks.bronze.runtime_checks import check_file_health
...
def _maintenance_arguments() -> list[str]:
    arguments = [
        "--table-path",
        os.getenv("BRONZE_TABLE_PATH", "s3a://bronze/mooc/bronze/mooc_events_raw"),
        "--app-name",
...
with DAG(
    ...
    schedule=os.getenv("BRONZE_MAINTENANCE_SCHEDULE", "0 */6 * * *"),
    catchup=False,
    ...
    post_check = PythonOperator(task_id="check_file_health", python_callable=check_file_health)
    end = EmptyOperator(task_id="end")
    start >> run_maintenance >> post_check >> end
