...
from domain.gold.common import (
...
def build_exam_anomaly_features(exam_df: DataFrame, system_df: DataFrame) -> DataFrame:
    exam_base = exam_df.withWatermark("time", EXAM_STATE_WATERMARK).withColumn(
    ...
    attempt_grouped = (
        exam_base.groupBy(*attempt_keys)
        .agg(
    ...
    security_grouped = (
        ...
        .agg(
            ...
            F.sum(
                F.when(F.col("security_event_type").contains("proctoring"), F.lit(1)).otherwise(
                    F.lit(0)
                )
    ...
    security_scored = (
        ...
        .withColumn("anomaly_domain", F.lit("exam"))
        .withColumn("anomaly_type", F.lit("multi_ip_login"))
        .withColumn("entity_type", F.lit("exam_attempt"))
        .withColumn(
            "entity_id",
            F.concat_ws("|", F.col("exam_id").cast("string"), F.col("attempt_id").cast("string")),
        )
        .withColumn("metric_name", F.lit("distinct_login_ips"))
        .withColumn("metric_value", F.col("distinct_login_ips").cast("double"))
        .withColumn("is_anomaly", F.col("distinct_login_ips") >= F.lit(2))
    )
