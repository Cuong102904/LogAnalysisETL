...
from domain.gold.common import safe_ratio
...
def build_quiz_performance_features(performance_df: DataFrame) -> DataFrame:
    base = (
        ...
        .withColumn(
            "score_ratio",
            safe_ratio(F.col("weighted_earned").cast("double"), F.col("weighted_possible").cast("double")),
        )
    )
    return base.groupBy("event_date", "course_id", "user_id", "problem_id", "problem_type").agg(
