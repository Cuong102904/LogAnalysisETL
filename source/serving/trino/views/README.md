...
These statements are applied automatically during Trino bootstrap and are the canonical definitions for the semantic layer.

Superset can consume the same semantic layer directly through Trino. The dashboard bootstrap under `serving/superset/bootstrap/` uses these view names as the source of truth for datasets and charts, so the BI layer stays code-driven instead of manually rebuilt in the UI.
